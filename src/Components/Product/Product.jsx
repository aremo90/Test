...
export default function Product({ product, index }) {
  ...
  return (
    ...
    <div className="grid grid-cols-1 overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
      <Link className="relative mx-3 mt-3 flex  overflow-hidden rounded-xl" to={"/productDetails/" + product._id}>
        ...
        {
          product?.priceAfterDiscount && <span className="absolute top-0 left-0 m-2 rounded-full bg-black px-2 text-center text-sm font-medium text-white">Sale</span>
        }
      </Link>
      <div className="mt-4 px-5 pb-5 grow flex flex-col justify-between">
        ...
        <Button disabled={isLoading} isLoading={isLoading} onPress={() => addItemToCard(product._id, setIsLoading)} 
        className="flex items-center justify-center rounded-md bg-black hover:bg-gray-600 px-5 py-2.5 text-center text-sm font-medium text-white focus:outline-none focus:ring-4 focus:ring-blue-300">
